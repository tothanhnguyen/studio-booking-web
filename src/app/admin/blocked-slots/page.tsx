...
export default async function BlockedSlotsPage() {
  ...
  return <section><h1 className="text-3xl font-semibold">Khung giờ bị chặn</h1><p className="mt-3 text-[var(--color-text-muted)]">Tạo block mới tại trang Lịch studio.</p><div className="mt-7 grid gap-3">{slots.length ? slots.map((slot) => <article key={slot.id} className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-4"><h2 className="font-semibold">{roomNames.get(slot.roomId)}</h2><p className="text-sm text-[var(--color-text-muted)]">{formatter.format(new Date(slot.startTime))} – {formatter.format(new Date(slot.endTime))}</p><p>{slot.reason}</p><form className="mt-2" action={deleteBlockedSlotAction.bind(null, slot.id)}><button className="text-sm font-semibold text-[var(--color-danger)]">Xóa block</button></form></article>) : <p>Chưa có khung giờ bị chặn.</p>}</div></section>;
}
