...
export function HoldCountdown({ expiresAt }: Readonly<{ expiresAt: string }>) {
  ...
  return <p role="timer" className="text-lg font-semibold text-[var(--color-accent)]">Giữ chỗ còn {String(Math.floor(totalSeconds / 60)).padStart(2, "0")}:{String(totalSeconds % 60).padStart(2, "0")}</p>;
}
